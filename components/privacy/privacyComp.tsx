...
const PrivacyComp: React.FC<Props> = ({ title, description }) => {
    const [open, setOpen] = React.useState(false);
    return (
        <div className='my-6 flex flex-col items-center w-full relative'>
            <div className='font-header cursor-pointer flex shadow-card-upload-black md:w-[605px] w-[calc(100vw-50px)] text-2xl font-semibold items-center justify-between bg-gradient-to-t from-[#000000cb] to-[#2ca9cf55] py-2 text-[white] px-8 rounded-lg z-30 tracking-wider' onClick={() => setOpen(!open)}>
                {title}
